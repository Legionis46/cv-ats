...
export default function CandidateModal({
  ...
  // Sync when initialCandidate changes
  useEffect(() => {
    if (initialCandidate) {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      // Don't show error if we already have initialCandidate
      if (!initialCandidate) {
        toast.error('Aday detayları yüklenemedi.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const updateStatus = async (status: CandidateStatus) => {
    ...
    const updatedCandidate = { ...candidate, status, updated_at: new Date().toISOString() };
            ...
            <div className="bg-gradient-to-r from-slate-800 to-slate-900 p-6 text-white flex-shrink-0">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0">
                  <div
                    className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${avatarColor(
                      candidate.name
                    ...
                    {candidate.last_position && (
                      <p className="text-slate-300 text-sm mt-0.5 truncate">
                        ...
                        {candidate.last_company && (
                          <span className="text-slate-400"> @ {candidate.last_company}</span>
                        )}
                      </p>
                    )}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {candidate.file_path && (
                    <button
                      onClick={() =>
                        window.open(`/api/candidates/${candidate.id}/file`, '_blank')
                      }
                        ...
                        {candidate.graduation_year && (
                          <div className="flex items-center gap-1 mt-1.5">
                            ...
                            <span className="text-xs text-violet-600">
                              Mezuniyet: {candidate.graduation_year}
                            </span>
                          </div>
                        )}
                      </div>
